...
from investigatinator.config import get_cisa_kev_path, knowledge_update_command
from investigatinator.enrichment.models import (
    InputValidationError,
    error_response,
    success_response,
)
from investigatinator.knowledge.attack import KnowledgeLoadError
...
def load_kev_catalog(path: Path | None = None) -> KevCatalog:
    """Load and parse the local CISA KEV catalog."""
...
def _search_entries(catalog: KevCatalog, query: str, limit: int) -> list[dict[str, Any]]:
