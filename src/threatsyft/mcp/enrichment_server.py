...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "failure. It returns no verdict and no confidence score, per source or "
        "overall - each provider's own fields come back under that provider's own "
        "names, and judgement is the caller's. Use enrichment_status to check "
...
# Reads the process environment for key presence. Despite living on the server
# that holds the keys, it calls nothing.
