# ThreatSyft Usage Guide

This guide shows the fastest ways to test ThreatSyft from the terminal and from an MCP-capable AI client.
...
Knowledge MCP lookups are local-only at runtime. They read `~/.threatsyft/knowledge/attack/enterprise-attack.json` by default and do not call MITRE during normal tool use.
...
D3FEND MCP lookups are local-only at runtime. They read `~/.threatsyft/knowledge/d3fend/d3fend.json` by default and map ATT&CK behavior to defensive technique context.
...
CISA KEV MCP lookups are local-only at runtime. They read `~/.threatsyft/knowledge/cisa/known_exploited_vulnerabilities.json` by default and do not call CISA during normal tool use.
...
LOLBAS MCP lookups are local-only at runtime. They read `~/.threatsyft/knowledge/lolbas/lolbas.json` by default and return defensive context while omitting raw command examples.
...
ThreatSyft's MCP servers are local stdio processes. Install the package in
...
/absolute/path/to/.venv/bin/threatsyft-knowledge-mcp
/absolute/path/to/.venv/bin/threatsyft-research-mcp
```
...
```text
Use ThreatSyft enrichment_status and tell me which enrichment providers are configured. Do not print secret values.
```
...
```text
Use ThreatSyft research_feed_status and tell me which RSS feeds are configured.
```
...
```text
Use ThreatSyft attack_search for PowerShell with a limit of 5.
```

```text
Use ThreatSyft attack_tactic_lookup on Initial Access and list the related techniques.
```

```text
Use ThreatSyft attack_defense_mapping on T1059 and explain the defensive techniques.
```

```text
Use ThreatSyft d3fend_lookup on File Analysis.
```
...
```text
Use ThreatSyft cve_lookup on CVE-2024-3400 and explain the NVD severity and CISA fields.
```
...
```text
Use ThreatSyft kev_lookup on CVE-2024-3400 and tell me if it appears in CISA KEV.
```
...
```text
Use ThreatSyft lolbas_search for T1105 with a limit of 5.
```

```text
Use ThreatSyft knowledge_status and tell me which local snapshots are ready.
```
...
```text
Use ThreatSyft research_article_summary on https://example.com/report and summarize the key public-report context.
```

```text
Use ThreatSyft research_article_iocs on https://example.com/report and list the extracted indicators.
```
...
If RDAP or WHOIS seems slow, keep `THREATSYFT_TIMEOUT_SECONDS` at `15` or temporarily increase it in `.env`.
